...
export const FiscalService = {
  ...
  validateDoc: (docType: string, docNumber: string): { valid: boolean; error?: string } => {
    ...
    if (docType === 'RUC') {
      ...
      if (!['10', '20', '15', '17'].includes(docNumber.substring(0, 2))) return { valid: false, error: 'RUC INVÁLIDO (PREFIJO)' };
    }
    return { valid: true };
  },
  ...
  getThermalTemplate: (order: OrderHistoryItem): string => {
    ...
    const paymentsSection = order.payments ? order.payments.map(p => {
