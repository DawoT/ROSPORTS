    ...
    <div className='pt-8 md:pt-12 pb-32 px-6 sm:px-10 lg:px-16 max-w-[1920px] mx-auto animate-in fade-in duration-700'>
      ...
      <div className='flex flex-col xl:flex-row xl:items-end justify-between gap-10 mb-16'>
        <div className='space-y-4'>
          ...
          <h2 className='font-space text-5xl md:text-6xl font-bold text-content-primary uppercase tracking-tighter'>
          ...
          <p className='text-content-secondary text-sm font-medium uppercase tracking-widest max-w-xl leading-relaxed'>
            Descubre <span className='text-content-primary font-bold'>{stats.visible}</span> modelos
            exclusivos seleccionados para los amantes de las zapatillas originales.
          </p>
        </div>
      ...
      {products.length > 0 ? (
        ...
        <div className='py-40 glass rounded-[4rem] border-content-muted/10 border-dashed border-2 flex flex-col items-center justify-center text-center space-y-8'>
          ...
          <p className='text-[10px] font-black text-content-muted uppercase tracking-[0.5em]'>
          ...
          <button
            ...
            className='px-10 py-4 glass text-blue-500 border-blue-500/20 rounded-2xl text-[9px] font-black uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all'
          >
            LIMPIAR FILTROS
          </button>
        </div>
      ...
      {isCompareHubOpen && (
        <ComparisonHub
          ...
          onAddToCart={(p) => {
            addToCart(p, 40, p.colors?.[0] || '#000');
            setIsCompareHubOpen(false);
          }}
        />
      )}
