...
const CommandCenter: React.FC = () => {
  ...
  const totalGMV = useMemo(
    () =>
      customers.reduce(
        (acc, c) => acc + (c.purchaseHistory?.reduce((sum, h) => sum + h.total, 0) || 0),
        0,
      ),
    [customers],
  );
    ...
    () =>
      products.reduce(
        (acc, p) =>
          acc +
          (p.variants?.reduce(
            (sum, v) => sum + v.inventoryLevels.reduce((s, l) => s + l.reserved, 0),
            0,
          ) || 0),
        0,
      ),
      ...
      <div className='flex flex-col xl:flex-row xl:items-end justify-between gap-8'>
          ...
          <div className='flex items-center gap-3 text-blue-500'>
            ...
            <div className='flex flex-col'>
              ...
              <h2 className='font-space text-4xl font-bold text-content-primary uppercase tracking-tighter leading-none'>
                Panel <span className='text-gradient'>Maestro</span>
              </h2>
            </div>
          </div>
        </div>
        ...
        <div className='flex gap-4'>
          <div className='glass px-8 py-5 rounded-[2rem] border-content-muted/10 space-y-1 bg-surface shadow-sm'>
            ...
            <p className='text-xs font-mono font-bold text-emerald-600 dark:text-emerald-500 truncate max-w-[200px]'>
              {lastIntegrityHash}
            </p>
          </div>
        </div>
        ...
        {[
          {
            ...
            val: `S/ ${totalGMV.toLocaleString()}`,
            icon: DollarSign,
            color: 'text-emerald-500',
          },
          {
            label: 'Pares Reservados',
            val: `${activeReserves} UN`,
            icon: Package,
            color: 'text-blue-500',
          },
          ...
          <div
            key={i}
            className='glass-card rounded-[2.5rem] p-8 border-content-muted/10 space-y-4 group hover:border-blue-500/20 transition-all bg-surface shadow-sm'
          >
            <div className='flex justify-between items-start'>
              <div
                className={`w-12 h-12 glass rounded-2xl flex items-center justify-center ${stat.color} bg-current/5 group-hover:scale-110 transition-transform`}
              >
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-10'>
          ...
          <div className='flex-1 relative z-10 flex items-center justify-center'>
            <div className='w-full max-w-lg aspect-video glass rounded-[3rem] border-content-muted/10 relative overflow-hidden group bg-main/20'>
              ...
              <div className='absolute inset-0 flex items-center justify-center pointer-events-none'>
                <p className='text-[8px] font-black text-content-muted uppercase tracking-[1em] opacity-40'>
                  Actualizando Mapa de Tiendas...
                </p>
              </div>
            </div>
          </div>
        ...
        <div className='glass-card rounded-[3rem] p-10 border-content-muted/10 space-y-10 bg-surface shadow-sm'>
          <div className='flex items-center gap-3 text-emerald-600 dark:text-emerald-500'>
            ...
            <h4 className='text-xl font-space font-bold uppercase text-content-primary'>
              Seguridad de Datos
            </h4>
          </div>
          <div className='space-y-4'>
            {auditLogs.slice(0, 5).map((log) => (
              <div
                key={log.id}
                className='p-4 glass rounded-2xl border-content-muted/10 flex gap-4 items-center bg-main/40 hover:border-emerald-500/30 transition-all'
              >
                ...
                <div className='flex-1 min-w-0'>
                  ...
                  <p className='text-[8px] font-mono text-content-muted truncate'>
                    {log.integrityHash}
                  </p>
                </div>
              </div>
