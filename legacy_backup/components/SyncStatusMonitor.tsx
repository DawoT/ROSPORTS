      ...
      <div className='glass px-6 py-4 rounded-[2rem] border-content-muted/10 bg-surface/90 backdrop-blur-xl shadow-2xl flex items-center gap-5 transition-all hover:pr-8 hover:border-blue-500/30'>
        <div className='relative'>
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${
              dbHealth.status === 'SYNCHRONIZED'
                ? 'bg-emerald-500/10 text-emerald-500'
                : dbHealth.status === 'PENDING_PUSH'
        ...
        <div className='flex flex-col'>
          <div className='flex items-center gap-2'>
            <span className='text-[9px] font-black uppercase tracking-widest text-content-primary'>
              Tienda Sincronizada
            </span>
            {dbHealth.status === 'SYNCHRONIZED' ? (
              <CheckCircle className='w-3 h-3 text-emerald-500' />
            ) : (
              <CloudSync className='w-3 h-3 text-blue-500 animate-pulse' />
            )}
          </div>
          <p className='text-[7px] font-bold text-content-muted uppercase tracking-tighter'>
            Última actualización: {new Date(dbHealth.lastGlobalSync).toLocaleTimeString()}
          </p>
        </div>
