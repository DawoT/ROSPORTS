  ...
  const avgMargin = useMemo(() => {
    ...
    return (
      validProducts.reduce(
        (acc, p) => acc + FinanceService.calculateProfitability(p.price, p.cost!),
        0,
    ...
    <div className='space-y-12 animate-in fade-in duration-700 pb-20'>
          ...
          <div className='flex items-center gap-3 text-amber-500'>
            ...
            <div className='flex flex-col'>
              ...
              <h2 className='font-space text-4xl font-bold dark:text-white uppercase tracking-tighter leading-none'>
                Fiscal <span className='text-amber-500'>Metrics</span>
              </h2>
            </div>
          </div>
        </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8'>
        {[
          {
            label: 'Total GMV (PEN)',
            val: FinanceService.formatCurrency(totalGMV),
            icon: DollarSign,
            color: 'text-emerald-500',
          },
          {
            label: 'Net Revenue (Ex-Tax)',
            val: FinanceService.formatCurrency(totalGMV / 1.18),
            icon: BarChart3,
            color: 'text-blue-500',
          },
          {
            label: 'IGV Payable (18%)',
            val: FinanceService.formatCurrency(totalGMV - totalGMV / 1.18),
            icon: ShieldCheck,
            color: 'text-red-500',
          },
          {
            label: 'Avg Portfolio Margin',
            val: `${avgMargin.toFixed(1)}%`,
            icon: TrendingUp,
          ...
          <div
            key={i}
            className='glass-card rounded-[2.5rem] p-8 border-white/5 space-y-4 hover:border-amber-500/30 transition-all group'
          >
            ...
            <div>
              ...
              <p className='text-2xl font-bold font-space dark:text-white text-slate-900'>
                {stat.val}
              </p>
            </div>
          </div>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-10'>
        <div className='lg:col-span-2 glass-card rounded-[3rem] p-12 border-white/5 relative overflow-hidden flex flex-col min-h-[500px]'>
          ...
          <div className='flex justify-between items-center mb-12 relative z-10'>
            <h3 className='text-[10px] font-black text-amber-500 uppercase tracking-[0.5em]'>
              Capital Rotation History
            </h3>
        ...
        <div className='glass-card rounded-[3rem] p-10 border-white/5 space-y-10'>
          ...
          <div className='p-8 glass rounded-[2.5rem] border-white/5 text-center space-y-4'>
            ...
            <p className='text-[10px] font-black text-slate-500 uppercase tracking-widest leading-relaxed'>
              Tax compliance report generated for SAT/SUNAT. No audit flags detected.
            </p>
            <button className='text-[9px] font-black text-blue-500 uppercase tracking-widest border-b border-blue-500/20'>
