    ...
    <div className='space-y-10 animate-in fade-in duration-700 pb-32'>
          ...
          <div className='flex items-center gap-3 text-blue-500'>
            ...
            <div className='flex flex-col'>
              ...
              <h2 className='font-space text-4xl font-bold text-content-primary uppercase tracking-tighter'>
                Ubicaciones & <span className='text-gradient'>Sucursales</span>
              </h2>
            </div>
          </div>
        </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8'>
        {inventoryNodes.map((node) => (
          <div
            key={node.id}
            className='glass-card rounded-[3rem] p-10 border-content-muted/10 space-y-8 flex flex-col bg-surface hover:border-blue-500/30 transition-all group shadow-sm'
          >
            <div className='flex justify-between items-start'>
              <div
                className={`w-16 h-16 glass rounded-2xl flex items-center justify-center ${node.type === 'store' ? 'text-blue-500' : 'text-purple-500'} bg-main border border-content-muted/10 shadow-xl group-hover:scale-110 transition-transform`}
              >
                {node.type === 'store' ? (
            ...
            <div className='space-y-2'>
              ...
              <h3 className='text-2xl font-space font-bold text-content-primary uppercase leading-none tracking-tighter'>
                {node.name}
              </h3>
            </div>
            ...
            <div className='space-y-4 pt-6 border-t border-content-muted/10'>
              <div className='flex items-start gap-4'>
                ...
                <p className='text-[11px] font-bold text-content-secondary uppercase leading-relaxed'>
                  {node.address}
                </p>
              </div>
            ...
            <div className='pt-4 flex items-center justify-between'>
              <div className='flex items-center gap-2'>
                <ShieldCheck
                  className={`w-4 h-4 ${node.allowsPublicSales ? 'text-emerald-500' : 'text-red-500'}`}
                />
                <span className='text-[8px] font-black text-content-muted uppercase tracking-widest'>
                  {node.allowsPublicSales ? 'VENTA PÚBLICO' : 'SOLO ALMACÉN'}
                </span>
              </div>
              <div className='flex gap-2'>
                <button
                  ...
                  className='p-3 glass rounded-xl border-content-muted/10 text-content-muted hover:text-blue-500 transition-all shadow-sm'
                >
...
const BranchEditor: React.FC<{
  branch: InventoryNode | null;
  onClose: () => void;
  onSave: (b: InventoryNode) => void;
}> = ({ branch, onClose, onSave }) => {
  const [form, setForm] = useState<Partial<InventoryNode>>(
    branch || {
      name: '',
      type: 'store',
      address: '',
      ...
      <div className='relative w-full max-w-2xl glass-card rounded-[3.5rem] border-content-muted/10 p-12 space-y-10 animate-in slide-in-from-bottom-12 duration-700 bg-surface shadow-2xl overflow-y-auto max-h-[90vh] custom-scrollbar'>
        <div className='flex items-center gap-4'>
          ...
          <h3 className='text-3xl font-space font-bold text-content-primary uppercase tracking-tighter'>
            {branch ? 'Actualizar Nodo' : 'Nuevo Nodo Físico'}
          </h3>
        </div>
        ...
        <div className='space-y-6'>
          <div className='grid grid-cols-2 gap-6'>
            <div className='space-y-2'>
              ...
              <input
                ...
                placeholder='EJ: NODO SUR JOCKEY'
              />
            </div>
            <div className='space-y-2'>
              <label className='text-[9px] font-black text-content-muted uppercase px-2 tracking-widest'>
          ...
          <div className='grid grid-cols-2 gap-6'>
            <div className='space-y-2'>
              ...
              <input
                ...
                onChange={(e) => setForm({ ...form, responsible: e.target.value.toUpperCase() })}
                className='w-full p-5 glass border-content-muted/10 rounded-2xl text-content-primary bg-main/50 text-xs font-bold outline-none focus:border-blue-500 shadow-sm'
              />
            </div>
            <div className='space-y-2'>
          ...
          >
            <div className='flex items-center gap-3'>
              <ShieldCheck
                className={`w-5 h-5 ${form.allowsPublicSales ? 'text-emerald-500' : 'text-content-muted'}`}
              />
              <span className='text-[10px] font-black text-content-muted uppercase tracking-widest group-hover:text-content-primary transition-colors'>
                Permite Venta al Público Directa
              </span>
            </div>
            <div
              className={`w-10 h-5 rounded-full transition-all relative ${form.allowsPublicSales ? 'bg-emerald-500' : 'bg-content-muted/20'}`}
            >
              <div
                className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${form.allowsPublicSales ? 'left-6' : 'left-1'}`}
              />
            </div>
          </button>
        </div>
        ...
        <div className='flex gap-4'>
          ...
          <button
            onClick={() =>
              onSave({ ...form, id: branch?.id || `NODE-${Date.now()}` } as InventoryNode)
            }
