...
const PromotionalSlider: React.FC<SliderProps> = ({ onShopClick }) => {
  ...
  return (
    <section className="relative min-h-[75vh] md:min-h-[85vh] flex items-center overflow-hidden bg-main transition-colors duration-700" aria-label="Novedades de la temporada">
      {/* Background Grid & Effects */}
      <div className="absolute inset-0 technical-grid opacity-10 pointer-events-none" aria-hidden="true" />
      <div className="absolute top-0 right-0 w-1/2 h-full dark:bg-brand-blue/[0.03] bg-brand-blue/[0.05] blur-[120px] animate-pulse pointer-events-none" aria-hidden="true" />
      ...
      <div className="max-w-[1920px] mx-auto w-full px-6 sm:px-16 flex relative z-10 py-12 md:py-20">
         <div className="grid grid-cols-1 lg:grid-cols-2 items-center gap-12 md:gap-20">
            <div className="space-y-8 md:space-y-12 animate-in slide-in-from-left-10 duration-1000">
               <div key={SLIDES[current].id} className="space-y-6 md:space-y-8 animate-in fade-in duration-700">
                  <div className="flex items-center gap-4">
                     <div className="w-12 h-px bg-brand-blue" />
                     <span className="text-brand-blue text-[10px] font-black uppercase tracking-[0.6em]">{SLIDES[current].label}</span>
                  </div>
            ...
            <div className="relative group hidden lg:block h-[600px] xl:h-[700px]" aria-hidden="true">
               <div key={SLIDES[current].id} className="absolute inset-0 flex items-center justify-center animate-in zoom-in-95 fade-in duration-1000">
                  <div className="absolute inset-0 dark:bg-brand-blue/5 bg-brand-blue/5 blur-[150px] rounded-full scale-110" />
                  <img src={SLIDES[current].img} alt="" className="relative z-10 w-full h-auto max-h-full object-contain drop-shadow-[0_40px_80px_rgba(0,0,0,0.3)] dark:drop-shadow-[0_50px_100px_rgba(0,0,0,0.8)] transition-transform group-hover:scale-105 duration-1000" />
               </div>
            </div>
         </div>
      </div>
