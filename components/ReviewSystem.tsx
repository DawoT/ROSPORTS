    ...
    <div className='space-y-12'>
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-10 items-center'>
        <div className='space-y-4 text-center lg:text-left'>
          <div className='space-y-1'>
            <h4 className='text-[9px] font-black text-brand-blue uppercase tracking-[0.5em]'>
              Experiencia de Comunidad
            </h4>
            <h3 className='text-3xl md:text-4xl font-space font-bold text-content-primary uppercase tracking-tighter'>
              Lo que dicen nuestros clientes
            </h3>
          </div>
          <div className='flex items-center justify-center lg:justify-start gap-5'>
            <span className='text-6xl md:text-7xl font-space font-bold text-content-primary leading-none'>
              {avgRating.toFixed(1)}
            </span>
            <div className='space-y-1'>
              <div className='flex gap-1'>
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    className={`w-4 h-4 ${i < Math.floor(avgRating) ? 'text-amber-500 fill-current' : 'text-content-muted opacity-20'}`}
                  />
                ))}
              </div>
              <p className='text-[8px] font-black text-content-secondary uppercase tracking-widest'>
                {reviews.length} Comentarios verificados
              </p>
            </div>
          </div>
        </div>
        ...
        <div className='lg:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6'>
          {[
            ...
            <div
              key={stat.label}
              className='p-5 md:p-6 glass rounded-[2rem] border-content-muted/10 space-y-3 bg-content-muted/[0.01]'
            >
      ...
      <div className='space-y-4'>
        {reviews.map((review) => (
          <div
            key={review.id}
            className='p-5 md:p-6 glass rounded-[2rem] border-content-muted/10 flex flex-col md:flex-row gap-6 group hover:border-brand-blue/20 transition-all bg-content-muted/[0.01]'
          >
            <div className='flex-1 space-y-4'>
                ...
                <div className='flex items-center gap-3'>
                  ...
                  <div className='flex items-center gap-3'>
                    ...
                    <span className='text-[8px] font-black text-content-muted uppercase tracking-widest'>
                      {TechnicalFormatter.date(review.date)}
                    </span>
                  </div>
                </div>
              ...
              <div className='space-y-3'>
                <div className='flex gap-1'>
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`w-3.5 h-3.5 ${i < review.rating ? 'text-amber-500 fill-current' : 'text-content-muted opacity-20'}`}
                    />
                  ))}
                </div>
                <p className='text-content-secondary text-sm leading-relaxed font-medium'>
                  "{review.comment}"
                </p>
              </div>
              ...
              <div className='flex flex-wrap gap-6 pt-2'>
                <div className='flex items-center gap-2'>
                  ...
                  <span className='text-[9px] font-bold text-content-primary uppercase'>
                    Perfecto
                  </span>
                </div>
                <div className='flex items-center gap-2'>
