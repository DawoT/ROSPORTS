    ...
    <div className="space-y-6 animate-in fade-in duration-1000">
      ...
      >
        ...
        <div className="absolute inset-0 z-20 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-700">
           ...
           <div className="absolute top-1/2 left-8 -translate-y-1/2 flex flex-col gap-1">
              {[...Array(5)].map((_, i) => <div key={i} className="w-1 h-1 bg-brand-blue/40 rounded-full" />)}
           </div>
        </div>
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar px-2">
         {images.map((img, i) => (
           ...
           >
              <img src={img} className="w-full h-full object-contain transition-transform group-hover:rotate-6" alt={`Thumbnail ${i + 1}`} />
              {activeIdx === i && <div className="absolute bottom-0 left-0 w-full h-1 bg-brand-blue animate-pulse" />}
           </button>
         ))}
      </div>
    </div>
