...
const FOCUS_RING =
  'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 focus-visible:ring-offset-main';
...
export const TechnicalBadge: React.FC<{
  ...
  return (
    <span
      role='status'
      className={`px-3 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-[0.2em] border ${styles[variant]} transition-all animate-in fade-in shadow-sm`}
    >
      {children}
    </span>
  );
        ...
        <thead>
          <tr className='bg-content-muted/[0.03] border-b border-content-muted/10'>
            {columns.map((col, idx) => (
              <th
                key={idx}
                className='px-8 py-6 text-[9px] font-black text-content-muted uppercase tracking-widest whitespace-nowrap'
              >
                {col.label}
              </th>
            ))}
...
export const EnterpriseButton: React.FC<
  React.ButtonHTMLAttributes<HTMLButtonElement> & {
    variant?: 'primary' | 'ghost' | 'offer';
    size?: 'sm' | 'md' | 'lg';
  }
> = ({ children, variant = 'primary', size = 'md', className, ...props }) => {
  ...
  const sizes = {
    sm: 'px-6 py-3 text-[8px]',
    md: 'px-10 py-5 text-[9px]',
    lg: 'px-12 py-6 text-[10px]',
  };
  return (
...
export const EnterpriseIconButton: React.FC<
  React.ButtonHTMLAttributes<HTMLButtonElement> & {
    icon: React.ReactNode;
    label: string;
    variant?: 'glass' | 'blue' | 'simple';
  }
> = ({ icon, label, variant = 'glass', className, ...props }) => {
  const variants = {
    glass:
      'glass border-content-muted/10 text-content-muted hover:text-brand-blue hover:border-brand-blue',
    blue: 'bg-blue-600 text-white shadow-lg shadow-blue-600/20',
    simple: 'text-content-muted hover:text-brand-blue',
  };
  return (
    <button
      className={`p-3.5 rounded-2xl transition-all flex items-center justify-center ${variants[variant]} ${FOCUS_RING} ${TOUCH_TARGET} ${className}`}
      aria-label={label}
      title={label}
      {...props}
    >
...
export const EnterpriseInput: React.FC<
...
> = ({ label, error, icon, className, ...props }) => {
  ...
  return (
    <div className='space-y-2 group w-full'>
      <label
        htmlFor={id}
        className='text-[10px] font-black uppercase tracking-widest text-content-secondary px-2 flex items-center gap-2 group-focus-within:text-blue-500 transition-colors'
      >
        {icon}
        {label}
      </label>
      <input
        id={id}
        className={`w-full px-6 py-5 glass border-content-muted/20 rounded-2xl outline-none focus:border-blue-500/50 transition-all font-bold text-sm ${error ? 'border-red-500/50' : ''} ${className}`}
        {...props}
      />
      {error && (
        <p className='text-[9px] font-black text-red-600 uppercase tracking-widest px-2'>{error}</p>
      )}
    </div>
  );
};
