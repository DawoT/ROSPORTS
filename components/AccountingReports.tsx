...
import { 
  FileText, Landmark, Users, TrendingUp, Download, 
  Search, Filter, Calendar, Zap, DollarSign,
  PieChart, ArrowUpRight, CheckCircle, ShieldCheck
} from 'lucide-react';
...
const AccountingReports: React.FC = () => {
  ...
  const cashAuditData = useMemo<CashAuditEntry[]>(() => {
     const historical: CashAuditEntry[] = [
       { id: 'CS-PREV-01', operator: 'Admin User', date: '2025-05-10', open: 500, cash: 1200, card: 850, other: 120, expected: 2670, actual: 2670, diff: 0 },
       { id: 'CS-PREV-02', operator: 'Sales Pro', date: '2025-05-09', open: 500, cash: 980, card: 420, other: 50, expected: 1950, actual: 1948.5, diff: -1.5 }
     ];
     return historical;
  }, []);
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-32">
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-8">
        <div className="space-y-4">
          <div className="flex items-center gap-3 text-emerald-500">
            ...
            <div className="flex flex-col">
               <span className="text-[9px] font-black uppercase tracking-[0.5em] text-content-muted">Financial Integrity Node</span>
               <h2 className="font-space text-4xl font-bold text-content-primary uppercase tracking-tighter leading-none">Reportes <span className="text-gradient">Contables</span></h2>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-4">
           <button 
            onClick={() => ReportingService.exportToCSV(salesBookData, `LIBRO_VENTAS_SUNAT_${dateRange}`)}
        ...
        <div className="space-y-8">
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="glass-card p-8 rounded-[2.5rem] border-content-muted/10 bg-surface space-y-2">
                 <p className="text-[8px] font-black text-content-muted uppercase tracking-widest">Base Imponible Mayo</p>
                 <p className="text-2xl font-space font-bold text-content-primary">S/ {(totalTaxPayable / 0.18).toLocaleString()}</p>
              </div>
              <div className="glass-card p-8 rounded-[2.5rem] border-content-muted/10 bg-surface space-y-2">
                 <p className="text-[8px] font-black text-content-muted uppercase tracking-widest">IGV a Pagar (18%)</p>
                 <p className="text-2xl font-space font-bold text-emerald-500">S/ {totalTaxPayable.toLocaleString()}</p>
              </div>
              <div className="glass-card p-8 rounded-[2.5rem] border-content-muted/10 bg-surface space-y-2">
                 <p className="text-[8px] font-black text-content-muted uppercase tracking-widest">Estado OSE/SUNAT</p>
           ...
           <EnterpriseDataTable<SalesBookEntry> 
            ...
            columns={[
              ...
              { key: 'tipo', label: 'Tipo', render: (i) => <span className="text-[10px] font-bold text-blue-500">{i.tipo}</span> },
              { key: 'id', label: 'Documento ID', render: (i) => <span className="text-[10px] font-mono font-bold">{i.id}</span> },
              { key: 'docNum', label: 'Doc Cliente' },
              { key: 'cliente', label: 'Razón Social', render: (i) => <span className="text-[10px] font-bold uppercase truncate max-w-[150px]">{i.cliente}</span> },
              { key: 'base', label: 'Base', render: (i) => <span className="text-xs font-bold">S/ {i.base.toFixed(2)}</span> },
              { key: 'igv', label: 'IGV', render: (i) => <span className="text-xs font-bold text-emerald-500">S/ {i.igv.toFixed(2)}</span> },
              { key: 'total', label: 'Total', render: (i) => <span className="text-sm font-bold text-content-primary">S/ {i.total.toFixed(2)}</span> }
            ]}
           />
        </div>
      )}
      ...
      {activeTab === 'cash-audit' && (
        ...
        <EnterpriseDataTable<CashAuditEntry> 
          ...
          columns={[
            ...
            { key: 'operator', label: 'Operador', render: (i) => <span className="text-xs font-bold uppercase">{i.operator}</span> },
            { key: 'cash', label: 'Ventas Cash', render: (i) => <span className="text-xs font-bold">S/ {i.cash.toFixed(2)}</span> },
            { key: 'expected', label: 'Sistema', render: (i) => <span className="text-xs font-bold text-blue-500">S/ {i.expected.toFixed(2)}</span> },
            { key: 'actual', label: 'Contado', render: (i) => <span className="text-xs font-bold text-content-primary">S/ {i.actual.toFixed(2)}</span> },
            { key: 'diff', label: 'Diferencia', render: (i) => (
              <span className={`text-xs font-black ${i.diff < 0 ? 'text-red-500' : 'text-emerald-500'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
           <div className="lg:col-span-2 space-y-8">
              ...
              <EnterpriseDataTable<CommissionEntry> 
                data={commissionData}
                columns={[
                  { key: 'seller', label: 'Operador Comercial', render: (i) => <span className="text-xs font-bold uppercase">{i.seller}</span> },
                  { key: 'orderCount', label: 'Volumen', render: (i) => <span className="text-xs font-bold">{i.orderCount} OP.</span> },
                  { key: 'totalSales', label: 'Ventas Netas', render: (i) => <span className="text-xs font-bold text-content-primary">S/ {i.totalSales.toFixed(2)}</span> },
                  { key: 'commission', label: 'Comisión (2.5%)', render: (i) => <span className="text-sm font-black text-emerald-600 dark:text-emerald-500">S/ {i.commission.toFixed(2)}</span> },
                  { key: 'actions', label: 'Estado Pago', render: () => <TechnicalBadge variant="amber">PENDING_SETTLE</TechnicalBadge> }
                ]}
              />
           </div>
           ...
           <div className="glass-card rounded-[3rem] p-10 border-content-muted/10 bg-surface shadow-sm space-y-10">
              ...
              <p className="text-xs text-content-secondary leading-relaxed font-medium italic">
                Las comisiones se calculan sobre el total neto de ventas confirmadas (no anuladas). El factor de incentivo actual está fijado en 2.5% para el Nodo Lima HQ.
              </p>
