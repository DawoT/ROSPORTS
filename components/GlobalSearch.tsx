...
import { Search, X, Package, ArrowRight, Zap, Target, Binary, Sparkles, AlertCircle } from 'lucide-react';
...
const GlobalSearch: React.FC<GlobalSearchProps> = ({ isOpen, onClose, products, onSelect }) => {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-start justify-center pt-[5vh] md:pt-[10vh] px-4 md:px-6">
      <div className="absolute inset-0 bg-main/90 backdrop-blur-[40px] animate-in fade-in" onClick={onClose} />
      ...
      <div className="relative w-full max-w-3xl glass-card rounded-[2.5rem] md:rounded-[3.5rem] overflow-hidden border border-content-muted/10 shadow-[0_40px_100px_rgba(0,0,0,0.5)] animate-in slide-in-from-top-10 bg-surface/98">
        <div className="p-6 md:p-10 border-b border-content-muted/10 bg-content-muted/[0.01]">
           <div className="relative flex items-center">
              <Search className={`absolute left-5 w-6 h-6 ${isAiLoading ? 'text-brand-blue animate-pulse' : 'text-content-muted'}`} />
              <input 
                ref={inputRef}
                type="text" 
              ...
              />
              <button onClick={onClose} className="p-3 glass rounded-xl text-content-muted hover:text-content-primary transition-all"><X /></button>
           </div>
           ...
           {suggestions.length > 0 && (
             <div className="flex items-center gap-4 mt-6 px-4 animate-in fade-in slide-in-from-left-2">
                ...
                <div className="flex flex-wrap gap-2">
                   {suggestions.map((s, i) => (
                     <button key={i} onClick={() => setQuery(s)} className="px-3 py-1 glass rounded-full text-[8px] font-black text-brand-blue uppercase tracking-widest hover:bg-brand-blue/10 transition-all border border-brand-blue/20">
                        {s}
                     </button>
                   ))}
                </div>
        ...
        <div className="p-4 md:p-6 max-h-[60vh] overflow-y-auto custom-scrollbar">
           {results.length > 0 ? (
             <div className="space-y-2">
                <p className="text-[8px] font-black text-content-muted uppercase tracking-[0.4em] px-4 mb-2">Modelos Encontrados</p>
                {results.map(p => (
                  <button 
                    key={p.id} 
                  ...
                  >
                    ...
                    <div className="flex-1 text-left">
                       <div className="flex items-center gap-2">
                          ...
                          <span className="text-[8px] font-bold text-content-muted uppercase tracking-widest">{p.category}</span>
                       </div>
                       <h4 className="text-lg font-bold text-content-primary uppercase leading-none mt-1 group-hover:text-brand-blue transition-colors">{p.name}</h4>
                    </div>
                    <div className="text-right shrink-0">
                       <p className="text-lg font-space font-bold text-content-primary">S/ {p.price.toFixed(2)}</p>
                       <span className="text-[7px] font-black text-emerald-500 uppercase tracking-widest">En Stock</span>
                    </div>
                  </button>
                ))}
             </div>
             ...
             <div className="py-20 flex flex-col items-center justify-center text-center space-y-6 opacity-30">
                ...
                <div className="space-y-1">
                   <p className="text-[10px] font-black uppercase tracking-[0.5em] text-content-primary">Sin Coincidencias</p>
                </div>
             </div>
           ) : (
             <div className="py-8 px-6">
                <h5 className="text-[9px] font-black text-content-muted uppercase tracking-[0.5em] mb-6">Explorar por Estilo</h5>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                   {['RUNNING ELITE', 'MODA URBANA', 'COLECCIÓN LIFESTYLE', 'CALZADO DEPORTIVO'].map(tag => (
                     <button key={tag} onClick={() => setQuery(tag)} className="p-5 glass rounded-2xl text-[10px] font-black uppercase tracking-widest hover:border-brand-blue/50 transition-all text-left flex items-center justify-between group text-content-secondary">
                        {tag}
