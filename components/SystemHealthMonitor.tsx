    ...
    <div className='space-y-10 animate-in fade-in duration-700 pb-20'>
      <div className='flex flex-col xl:flex-row xl:items-end justify-between gap-8'>
        ...
        <div className='flex gap-4'>
          <div
            className={`px-8 py-5 rounded-2xl border flex items-center gap-4 transition-all shadow-xl bg-surface ${networkStatus === 'CIRCUIT_OPEN' ? 'border-red-500/30' : 'border-emerald-500/30'}`}
          >
            {networkStatus === 'CIRCUIT_OPEN' ? (
            ...
            <div className='flex flex-col'>
              ...
              <span className={`text-xs font-bold uppercase ${getStatusColor(networkStatus)}`}>
                {networkStatus}
              </span>
            </div>
          </div>
        ...
        {report.map((mod) => (
          ...
          >
            <div className='flex justify-between items-start'>
              ...
              <TechnicalBadge
                variant={
                  mod.status === 'NOMINAL' ? 'emerald' : mod.status === 'DEGRADED' ? 'amber' : 'red'
                }
              >
                {mod.status}
              </TechnicalBadge>
            ...
            <div>
              ...
              <p className='text-[8px] font-mono text-content-muted mt-2'>
                LAST_SYNC: {new Date(mod.lastSync).toLocaleTimeString()}
              </p>
            </div>
          </div>
        ))}
      ...
      <div className='space-y-6'>
        <h4 className='text-[10px] font-black text-blue-500 uppercase tracking-[0.6em] px-4'>
        ...
        <EnterpriseDataTable<ModuleHealth>
          ...
          columns={[
            ...
            { key: 'label', label: 'Service Component' },
            {
              ...
              label: 'Health State',
              render: (item) => (
                <TechnicalBadge variant={item.status === 'NOMINAL' ? 'emerald' : 'red'}>
                  {item.status}
                </TechnicalBadge>
              ),
            },
            {
              ...
              render: (item) => (
                <span
                  className={`text-xs font-bold ${item.errors > 0 ? 'text-red-500' : 'text-content-muted'}`}
                >
            ...
            },
            {
              ...
              label: 'Round-trip',
              render: (item) => (
                <span className='text-xs font-bold text-content-secondary'>{item.latency}ms</span>
              ),
            },
          ]}
        />
