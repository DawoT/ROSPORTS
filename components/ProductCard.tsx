...
const ProductCard: React.FC<ProductCardProps> = ({ 
  product, isWishlisted, onAddToCart, onViewDetails, onQuickView, onToggleWishlist 
}) => {
  ...
  return (
    <article 
      className={`group relative glass rounded-[2.5rem] overflow-hidden transition-all duration-700 shadow-sm border border-content-muted/10 flex flex-col h-full ${
        isOutOfStock ? 'opacity-80 grayscale' : 'hover:-translate-y-4 hover:shadow-[0_40px_80px_rgba(0,0,0,0.15)] hover:border-brand-blue/30'
      }`}
    >
      {/* Strategic Offer Badge */}
      {hasOffer && (
        <div className="absolute top-0 left-0 z-40">
           <DealBadge label={`-${Math.round((1 - product.price/product.originalPrice)*100)}% RED ALERT`} />
        </div>
      )}
      ...
      <div className="absolute top-6 right-6 z-40 flex flex-col gap-3 translate-x-12 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-500">
        ...
        />
        <EnterpriseIconButton 
          icon={<Heart className={`w-5 h-5 ${isWishlisted ? 'fill-brand-offer text-brand-offer border-brand-offer' : ''}`} />}
          label="Favorites"
      ...
      >
        ...
        {isOutOfStock && (
          <div className="absolute inset-0 z-20 flex items-center justify-center bg-main/60 backdrop-blur-md">
             <span className="bg-content-primary text-main px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest shadow-2xl border border-white/10">Sold Out</span>
          </div>
        )}
      </div>
            ...
            {product.colors?.map(color => (
              <button 
                ...
                className={`w-9 h-9 rounded-xl border-2 transition-all shadow-md relative group ${
                  selectedColor === color ? 'border-brand-blue scale-110 shadow-blue-500/30' : 'border-transparent opacity-60 hover:opacity-100'
                }`} 
                style={{ backgroundColor: color }} 
              >
                 {selectedColor === color && <div className="absolute inset-1 border border-white/30 rounded-lg" />}
              </button>
            ))}
          </div>
              ...
              return (
                <button 
                  ...
                  className={`h-10 rounded-xl border text-[10px] font-black transition-all flex items-center justify-center ${
                    selectedSize === size ? 'bg-brand-blue border-brand-blue text-white shadow-xl' : 
        ...
        <div className="mt-auto pt-8 border-t border-content-muted/10 flex items-center justify-between">
          <div className="flex flex-col">
            <div className="flex items-baseline gap-3">
               <span className="text-3xl font-bold font-space text-content-primary tracking-tighter">S/ {product.price.toFixed(2)}</span>
               {hasOffer && <span className="text-sm font-bold text-content-muted line-through opacity-40">S/ {product.originalPrice?.toFixed(2)}</span>}
            </div>
            {hasOffer && (
          ...
          <div className="relative">
            ...
            >
              {isAddedFeedback ? <Check className="w-7 h-7 animate-in zoom-in" /> : <ShoppingCart className="w-7 h-7 group-hover/buy:scale-110 transition-transform" />}
            </button>
            ...
            {showTooltip && (
              <div className="absolute bottom-full mb-6 right-0 w-48 glass p-5 rounded-[1.5rem] border-brand-offer/40 text-center animate-in slide-in-from-bottom-4 shadow-2xl">
                <p className="text-[9px] font-black text-brand-offer uppercase tracking-widest leading-tight">Configuración Técnica Requerida</p>
              </div>
            )}
          </div>
        </div>
