...
const CashSessionManager: React.FC = () => {
  ...
  const handleOpen = () => {
    ...
    addAuditLog(
      ...
      'cash_session',
      session.id,
      `Apertura con S/ ${currentTotal.toFixed(2)}`,
    );
  ...
  if (step === 'report' && lastClosedSession) {
    ...
    const manualIn = lastClosedSession.movements
      .filter((m) => m.type === 'income')
      .reduce((a, b) => a + b.amount, 0);
    const manualOut = lastClosedSession.movements
    ...
    return (
      <div className='max-w-4xl mx-auto space-y-12 animate-in zoom-in-95 duration-500 pb-40'>
        <div className='flex items-center justify-between no-print'>
          ...
          <div className='flex gap-4'>
            <button
              ...
              className='px-8 py-4 bg-white text-black rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all shadow-2xl flex items-center gap-3'
            >
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-10'>
          ...
          <div className='space-y-8 no-print'>
            <div className='glass-card rounded-[3rem] p-10 border-white/5 space-y-8 bg-gradient-to-br from-blue-600/[0.03] to-transparent'>
              ...
              <div className='grid grid-cols-2 gap-6'>
                <div className='space-y-1'>
                  ...
                  <p
                    className={`text-4xl font-space font-bold ${lastClosedSession.difference! < 0 ? 'text-red-500' : 'text-emerald-500'}`}
                  >
            ...
            <div className='glass rounded-[2.5rem] border-white/5 p-8 space-y-6'>
              ...
              <div className='space-y-4'>
                {[
                  {
                    label: 'Inducción Inicial',
                    val: lastClosedSession.openingBalance,
                    type: 'pos',
                  },
                  {
                    label: 'Ventas en Efectivo',
                    val: lastClosedSession.systemExpected.cash,
                    type: 'pos',
                  },
                  ...
                  >
                    ...
                    <span
                      className={`text-sm font-bold ${op.type === 'pos' ? 'text-white' : 'text-red-500'}`}
                    >
          ...
          <div
            id='printable-report'
            className='bg-white text-black p-10 shadow-2xl font-mono text-[11px] space-y-8 max-w-[400px] mx-auto border border-black/10'
          >
            ...
            <div className='grid grid-cols-2 gap-y-1 text-[10px]'>
              <span>SESIÓN ID:</span>{' '}
              <span className='text-right font-bold'>{lastClosedSession.id}</span>
              <span>RESPONSABLE:</span>{' '}
              <span className='text-right font-bold uppercase'>{lastClosedSession.userName}</span>
              <span>APERTURA:</span>{' '}
              <span className='text-right'>
                {new Date(lastClosedSession.startTime).toLocaleTimeString()}
              </span>
              <span>CIERRE:</span>{' '}
              <span className='text-right'>
                {new Date(lastClosedSession.endTime!).toLocaleTimeString()}
              </span>
            </div>
            ...
            <div className='space-y-4'>
              ...
              <div className='grid grid-cols-2 gap-y-1'>
                ...
                <span className='text-right'>
                  S/ {lastClosedSession.systemExpected.cash.toFixed(2)}
                </span>
                <span>VENTAS CARD:</span>{' '}
                <span className='text-right'>
                  S/ {lastClosedSession.systemExpected.card.toFixed(2)}
                </span>
                <span>DIGITAL WALLET:</span>{' '}
                <span className='text-right'>
                  S/ {lastClosedSession.systemExpected.digital.toFixed(2)}
                </span>
                ...
                <span className='text-right font-bold'>
                  S/{' '}
                  {(
                    lastClosedSession.systemExpected.cash +
                    lastClosedSession.systemExpected.card +
                    lastClosedSession.systemExpected.digital
                  ).toFixed(2)}
                </span>
                <span>TRANSACCIONES:</span>{' '}
                <span className='text-right'>{lastClosedSession.systemExpected.salesCount}</span>
              </div>
            </div>
            ...
            <div className='space-y-4'>
              ...
              <div className='grid grid-cols-2 gap-y-1'>
                <span>(+) APERTURA:</span>{' '}
                <span className='text-right'>S/ {lastClosedSession.openingBalance.toFixed(2)}</span>
                <span>(+) COBROS EFECT.:</span>{' '}
                <span className='text-right'>
                  S/ {lastClosedSession.systemExpected.cash.toFixed(2)}
                </span>
                <span>(+/-) MOVIMIENTOS:</span>{' '}
                <span className='text-right'>S/ {(manualIn - manualOut).toFixed(2)}</span>
                <div className='col-span-2 h-px border-t border-black my-1' />
                <span className='font-bold'>ESPERADO:</span>{' '}
                <span className='text-right font-bold'>S/ {totalExpected.toFixed(2)}</span>
                <span className='font-bold'>CONTADO:</span>{' '}
                <span className='text-right font-bold'>
                  S/ {lastClosedSession.actualCashBalance?.toFixed(2)}
                </span>
                <div className='col-span-2 h-px border-t border-black my-1' />
                <span className='font-bold text-[14px]'>DIFERENCIA:</span>
                <span
                  className={`text-right font-bold text-[14px] ${lastClosedSession.difference! < 0 ? 'underline' : ''}`}
                >
                  S/ {lastClosedSession.difference!.toFixed(2)}
                </span>
              </div>
            ...
            <div className='space-y-4'>
              ...
              <div className='grid grid-cols-2 gap-y-0.5 text-[9px]'>
                {lastClosedSession.closingDenominations
                  ?.filter((d) => d.count > 0)
                  .map((d) => (
            ...
            <div className='pt-20 text-center space-y-12'>
              ...
              <div className='space-y-1'>
                <p className='text-[7px] opacity-60'>
                  AUTH_INTEGRITY_CERT: {lastClosedSession.integrityHash}
                </p>
    ...
    return (
      <div className='flex items-center justify-center min-h-[60vh] animate-in zoom-in-95'>
        <div className='glass-card rounded-[3.5rem] p-16 border-white/10 max-w-xl w-full text-center space-y-10'>
          ...
          <div className='space-y-4'>
            <h3 className='text-4xl font-space font-bold text-white uppercase tracking-tighter leading-none'>
              Gestión de <span className='text-blue-500'>Caja</span>
            </h3>
            <p className='text-slate-500 text-[10px] font-black uppercase tracking-[0.4em] leading-relaxed'>
              Protocolo de apertura de terminal necesario.
            </p>
          </div>
          <button
            onClick={() => {
              setTempDenoms(PERU_DENOMINATIONS.map((d) => ({ ...d, count: 0 })));
              setStep('opening');
            }}
            className='w-full py-6 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase tracking-[0.4em] shadow-xl hover:bg-blue-500 transition-all'
          >
      ...
      <div className='max-w-6xl mx-auto space-y-10 animate-in fade-in duration-500 pb-32'>
        <div className='flex flex-col md:flex-row justify-between items-end gap-6'>
          <div className='space-y-2'>
            <div className='flex items-center gap-3 text-blue-500'>
              ...
              <span className='text-[10px] font-black uppercase tracking-[0.5em]'>
                {step === 'opening' ? 'Conteo de Inicio' : 'Arqueo de Cierre'}
              </span>
            </div>
            <h2 className='text-4xl font-space font-bold text-white uppercase tracking-tighter'>
              Arqueo de <span className='text-gradient'>Divisas</span>
            </h2>
          </div>
          <div className='glass px-12 py-6 rounded-[2.5rem] border-emerald-500/20 bg-emerald-500/5 text-right'>
            ...
            <p className='text-5xl font-space font-bold text-emerald-500 leading-none mt-1'>
        ...
        <div className='grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4'>
          {tempDenoms.map((d, i) => (
            <div
              key={d.value}
              className='glass p-5 rounded-3xl border-white/5 space-y-4 group hover:border-blue-500/30 transition-all'
            >
              ...
              <div className='flex items-center justify-between gap-2 bg-black/40 p-2 rounded-2xl'>
                <button
                  ...
                  className='w-8 h-8 glass rounded-xl flex items-center justify-center text-slate-400 hover:text-red-500 transition-colors'
                >
                ...
                <button
                  ...
                  className='w-8 h-8 glass rounded-xl flex items-center justify-center text-slate-400 hover:text-emerald-500 transition-colors'
                >
        ...
        <div className='flex justify-end gap-6 pt-10 border-t border-white/5'>
          <button
            onClick={() => (activeCashSession ? setStep('active') : setStep('idle'))}
            className='px-10 py-5 glass text-slate-500 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:text-white'
          >
            VOLVER
          </button>
          <button
          ...
          >
            {step === 'opening' ? 'CONFIRMAR APERTURA' : 'EJECUTAR CIERRE'}{' '}
  ...
  if ((step === 'active' || !step || step === 'idle') && activeCashSession) {
    const expectedCash = CashService.calculateExpectedCash(activeCashSession);
    const manualIn = activeCashSession.movements
      .filter((m) => m.type === 'income')
      .reduce((a, b) => a + b.amount, 0);
    const manualOut = activeCashSession.movements
      .filter((m) => m.type === 'expense')
      .reduce((a, b) => a + b.amount, 0);
        ...
        <div className='flex flex-col xl:flex-row xl:items-end justify-between gap-8'>
            ...
            <div className='flex items-center gap-3 text-emerald-500'>
              ...
              <div className='flex flex-col'>
                ...
                <h2 className='font-space text-4xl font-bold dark:text-white uppercase tracking-tighter'>
                  Turno: <span className='text-gradient'>{activeCashSession.id}</span>
                </h2>
              </div>
            </div>
          </div>
          <div className='flex gap-4'>
            <button
              ...
              className='px-8 py-5 glass border-blue-500/20 text-blue-500 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-500/10 transition-all flex items-center gap-3'
            >
            ...
            <button
              ...
              className='px-10 py-5 bg-red-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl hover:bg-red-500 transition-all flex items-center gap-3'
            >
          ...
          <div className='lg:col-span-3 space-y-8'>
            <div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
              {[
                {
                  label: 'Inducción Inicial',
                  val: activeCashSession.openingBalance,
                  color: 'text-slate-400',
                  icon: Landmark,
                },
                {
                  label: 'Ventas en Efectivo',
                  val: activeCashSession.systemExpected.cash,
                  color: 'text-blue-500',
                  icon: Banknote,
                },
                {
                  label: 'Flujo Manual Neto',
                  val: manualIn - manualOut,
                  color: 'text-purple-500',
                  icon: History,
                },
                {
                  label: 'Esperado en Bandeja',
                  val: expectedCash,
                ...
                <div key={i} className='glass-card p-6 rounded-[2rem] border-white/5 space-y-3'>
                  ...
                  <div>
                    ...
                    <p className={`text-xl font-space font-bold ${s.color}`}>
                      S/ {s.val.toFixed(2)}
                    </p>
                  </div>
                </div>
                ...
                <table className='w-full text-left'>
                  <thead>
                    <tr className='bg-white/5 border-b border-white/5'>
                      <th className='px-8 py-4 text-[8px] font-black text-slate-500 uppercase'>
                        Timestamp
                      </th>
                      <th className='px-8 py-4 text-[8px] font-black text-slate-500 uppercase'>
                        Tipo
                      ...
                      <th className='px-8 py-4 text-[8px] font-black text-slate-500 uppercase text-right'>
                        Monto
                      </th>
                    </tr>
                  </thead>
                  <tbody className='divide-y divide-white/5'>
                    {activeCashSession.movements.map((m) => (
                      <tr key={m.id} className='hover:bg-white/[0.01]'>
                        <td className='px-8 py-4 text-[10px] font-mono text-slate-500'>
                          {new Date(m.timestamp).toLocaleTimeString()}
                        </td>
                        <td className='px-8 py-4'>
                          <span
                            className={`px-2 py-0.5 rounded text-[7px] font-black uppercase border ${m.type === 'income' ? 'text-emerald-500 border-emerald-500/20' : 'text-red-500 border-red-500/20'}`}
                          >
                            {m.type}
                          </span>
                        </td>
                        <td className='px-8 py-4 text-[10px] font-bold text-white uppercase'>
                          {m.concept}
                        </td>
                        <td className='px-8 py-4 text-[9px] font-black text-slate-500 uppercase'>
                          {m.authorizedBy}
                        </td>
                        <td
                          className={`px-8 py-4 text-right text-[11px] font-bold ${m.type === 'income' ? 'text-emerald-500' : 'text-red-500'}`}
                        >
                    ...
                    {activeCashSession.movements.length === 0 && (
                      <tr>
                        <td
                          colSpan={5}
                          className='py-20 text-center opacity-30 text-[9px] font-black uppercase tracking-[0.5em]'
                        >
              ...
              <div className='space-y-4'>
                ...
                <div className='flex justify-between items-center'>
                  ...
                  <span className='text-[10px] font-bold text-white'>
                    {new Date(activeCashSession.startTime).toLocaleTimeString()}
                  </span>
                </div>
              </div>
            </div>
    ...
    <div className='fixed inset-0 z-[500] flex items-center justify-center p-6'>
      ...
      <div className='relative w-full max-w-xl glass-card rounded-[3.5rem] border-white/10 p-12 space-y-10 animate-in zoom-in-95'>
        <div className='flex items-center gap-4'>
          ...
          <h3 className='text-3xl font-space font-bold text-white uppercase tracking-tighter'>
            Nuevo Flujo
          </h3>
        </div>
        <div className='space-y-6'>
          ...
          <div className='space-y-2'>
            <label className='text-[9px] font-black text-slate-500 uppercase px-2'>
          ...
          {form.concept === 'PAGO DE CRÉDITO ANTERIOR' && (
            <div className='space-y-2 animate-in slide-in-from-top-2'>
              <label className='text-[9px] font-black text-amber-500 uppercase px-2'>
                Vincular Cliente con Deuda
              </label>
              <select
                ...
                {customers
                  ...
                  .map((d) => (
                    <option key={d.id} value={d.id}>
                      {d.fullName} (S/ {d.creditBalance.toFixed(2)})
                    </option>
                  ))}
              </select>
            </div>
          ...
          <div className='space-y-2'>
            ...
            <input
              ...
              className='w-full p-5 glass rounded-2xl text-white text-2xl font-space font-bold outline-none'
              placeholder='0.00'
            />
          </div>
          <div className='space-y-2'>
            ...
            <input
              onChange={(e) => setForm({ ...form, authorizedBy: e.target.value.toUpperCase() })}
              className='w-full p-5 glass rounded-2xl text-white text-xs font-bold uppercase'
        ...
        <div className='flex gap-4'>
          <button
            onClick={onClose}
            className='flex-1 py-5 glass text-slate-500 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:text-white'
          >
            ABORTAR
          </button>
          <button
