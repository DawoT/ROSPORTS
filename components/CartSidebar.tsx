...
const CartSidebar: React.FC<CartSidebarProps> = ({ isOpen, onClose, onCheckout }) => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 right-0 h-full w-full max-w-md glass border-l border-content-muted/10 z-[810] transition-transform duration-500 transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
                ...
                <div className='flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-brand-blue'>
                  ...
                  <span>
                    {progressToFreeShipping >= 100
                      ? 'ACTIVO'
                      : `Faltan S/ ${(freeShippingThreshold - subtotal).toFixed(0)}`}
                  </span>
                </div>
                <div className='h-1.5 w-full bg-content-muted/10 rounded-full overflow-hidden'>
                  <div
                    className='h-full bg-brand-blue transition-all duration-1000 shadow-[0_0_15px_rgba(59,130,246,0.5)]'
                  ...
                  >
                    ...
                    <div className='flex-1 min-w-0 flex flex-col justify-between py-1'>
                      <div className='flex justify-between items-start gap-2'>
                        <h4 className='font-bold text-sm text-content-primary uppercase truncate font-space tracking-tight'>
                          {item.name}
                        </h4>
                        <button
                          onClick={() =>
                      ...
                      <div className='flex items-center justify-between mt-4'>
                        <div className='flex items-center gap-5 bg-surface rounded-xl p-1.5 px-4 border border-content-muted/10'>
                          <button
                            onClick={() =>
                              updateCartQuantity(item.id, item.selectedSize, -1, item.selectedColor)
                            }
                            className='hover:text-brand-blue min-h-[30px] min-w-[30px]'
            ...
            <div className='p-8 glass border-t border-content-muted/10 space-y-8 bg-gradient-to-t from-content-muted/[0.04] to-transparent'>
              <div className='flex justify-between items-end'>
                <div className='space-y-1'>
                  <span className='text-brand-blue font-black uppercase tracking-[0.4em] text-[10px]'>
                    Inversión Total
                  </span>
                  <p className='text-4xl font-space font-bold text-content-primary tracking-tighter'>
                    S/ {subtotal.toFixed(2)}
                  </p>
                </div>
                <div className='text-right'>
                  <p className='text-[8px] font-black text-emerald-500 uppercase flex items-center gap-1 justify-end'>
                    <Zap className='w-3 h-3 fill-current' /> +{Math.floor(subtotal)} Puntos
                  </p>
                </div>
              </div>
              <button
