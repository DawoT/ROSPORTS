...
import { Filter, X, ChevronDown, Check, Zap, DollarSign, Ruler, Palette, Globe } from 'lucide-react';
...
const FilterSidebar: React.FC<FilterSidebarProps> = ({ 
  isOpen, onClose, filters, setFilters, brands, categories 
}) => {
  ...
  return (
    <>
      <div className={`fixed inset-0 z-[150] bg-main/60 backdrop-blur-sm transition-opacity duration-500 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={onClose} />
      
      <aside className={`fixed top-0 left-0 h-full w-full max-w-xs glass border-r border-content-muted/10 z-[160] transition-transform duration-500 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col bg-main shadow-2xl`}>
        <div className="p-8 border-b border-content-muted/10 flex items-center justify-between bg-content-muted/[0.01]">
           <div className="flex items-center gap-4">
              ...
              <h2 className="text-xl font-space font-bold text-content-primary uppercase tracking-tighter">REFINAR</h2>
           </div>
           <button onClick={onClose} className="p-3 glass rounded-2xl text-content-muted hover:text-red-500 transition-all"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar pb-32">
           ...
           <div className="space-y-5">
              ...
              <div className="flex flex-col gap-2.5">
                 {categories.map(cat => (
                   <button 
                    key={cat} 
                    onClick={() => setFilters({...filters, categories: toggleItem(filters.categories, cat)})}
                   ...
                   >
                      {cat}
                      {filters.categories.includes(cat) ? <Check className="w-3.5 h-3.5" /> : <div className="w-4 h-4 rounded-full border-2 border-content-muted/20" />}
                   </button>
                 ))}
              </div>
           </div>
           ...
           <div className="pt-8 border-t border-content-muted/10">
              ...
              >
                 <span className="text-[10px] font-black text-content-muted uppercase tracking-widest group-hover:text-emerald-500 transition-colors">Disponibilidad Inmediata</span>
                 <div className={`w-11 h-6 rounded-full transition-all relative ${filters.inStock ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.4)]' : 'bg-content-muted/20'}`}>
                    <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${filters.inStock ? 'left-6' : 'left-1'}`} />
                 </div>
              </button>
           </div>
        </div>
        ...
        <div className="p-8 border-t border-content-muted/10 bg-content-muted/[0.04] flex gap-4">
           <button onClick={() => setFilters({ categories: [], brands: [], priceRange: [0, 2000], sizes: [], colors: [], inStock: false })} className="flex-1 py-4 glass text-[9px] font-black text-content-muted uppercase tracking-widest rounded-2xl hover:text-content-primary transition-all">LIMPIAR</button>
           <button onClick={onClose} className="flex-1 py-4 bg-brand-blue text-white text-[9px] font-black uppercase tracking-[0.3em] rounded-2xl shadow-xl hover:bg-blue-600 transition-all">VER RESULTADOS</button>
        </div>
      </aside>
    </>
  );
