    ...
    <div className='space-y-10 animate-in slide-in-from-right-12 duration-700 pb-20'>
      <div className='flex flex-col xl:flex-row xl:items-end justify-between gap-8'>
        <div className='space-y-4'>
          <button
            onClick={onBack}
            className='flex items-center gap-2 text-[10px] font-black text-slate-500 hover:text-blue-500 transition-all uppercase tracking-[0.3em]'
          >
          ...
          <div className='glass-card rounded-[3rem] p-12 border-white/5 relative overflow-hidden'>
            ...
            <div className='flex justify-between items-center mb-12 relative z-10'>
              <h4 className='text-[10px] font-black text-blue-500 uppercase tracking-[0.4em]'>
                Saldos por Nodo Operativo
              </h4>
            </div>
            <div className='space-y-6 relative z-10'>
              {product.variants?.[0]?.inventoryLevels.map((level) => (
                <div key={level.nodeId} className='flex items-center gap-6'>
                  <span className='w-32 text-[10px] font-black text-slate-500 uppercase tracking-widest'>
                    {level.nodeId}
                  </span>
                  <div className='flex-1 h-3 glass rounded-full overflow-hidden'>
                    <div
                      ...
                      style={{ width: `${(level.quantity / 50) * 100}%` }}
                    />
                  </div>
        ...
        <div className='space-y-8'>
          <div className='glass-card rounded-[3rem] p-10 border-white/5 flex flex-col items-center text-center space-y-6'>
            ...
            <div className='space-y-2'>
              ...
              <h3 className='font-space text-2xl font-bold text-white uppercase leading-none'>
                {product.name}
              </h3>
            </div>
