          ...
          return {
            ...
            purchaseHistory: c.purchaseHistory.map((h) =>
              h.orderId === order.orderId
                ? { ...h, status: 'voided', voidReason: reason, voidDate: new Date().toISOString() }
                : h,
            ),
          };
        }),
      ...
      <div className='relative w-full max-w-xl glass-card rounded-[3rem] border-red-500/20 p-12 space-y-10 animate-in zoom-in-95'>
        <div className='flex flex-col items-center text-center space-y-4'>
          ...
          <div className='space-y-2'>
            ...
            <p className='text-[10px] font-black text-red-500 uppercase tracking-[0.4em]'>
              Protocolo de Comunicación de Baja
            </p>
          </div>
        </div>
        ...
        <div className='space-y-4'>
          <label className='text-[10px] font-black text-slate-500 uppercase tracking-widest px-2'>
            JUSTIFICACIÓN TÉCNICA (Obligatorio SUNAT)
          </label>
          <textarea
            value={reason}
        ...
        <div className='grid grid-cols-2 gap-6'>
          <button
            onClick={onClose}
            className='py-5 glass text-slate-500 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:text-white'
          >
            DESCARTAR
          </button>
          <button
        ...
        <p className='text-[8px] text-slate-600 text-center uppercase tracking-widest leading-relaxed'>
          Esta acción es irreversible y notificará automáticamente a los servicios electrónicos
          vinculados.
        </p>
      </div>
    </div>
