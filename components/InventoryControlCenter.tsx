...
const InventoryControlCenter: React.FC<InventoryControlCenterProps> = ({
  ...
  const [transfers, setTransfers] = useState<StockTransfer[]>([
    {
      ...
      originNodeId: 'N-01',
      targetNodeId: 'N-02',
      items: [{ sku: 'RS-VEL-X1-38-RED', quantity: 5, name: 'Velocity X-1 Pro' }],
      status: 'shipped',
      createdAt: '2025-05-12 10:00',
      authorizedBy: 'ADMIN MASTER',
    },
    {
      id: 'TR-1002',
      originNodeId: 'N-01',
      targetNodeId: 'N-03',
      items: [{ sku: 'RS-GRA-K2-41-BLK', quantity: 2, name: 'Gravity King' }],
      status: 'pending',
      createdAt: '2025-05-12 14:30',
      authorizedBy: 'ADMIN MASTER',
    },
  ]);
  ...
  const handleConfirmReception = (transferId: string) => {
    ...
    addAuditLog(
      ...
      'transfer',
      transferId,
      `Stock recibido en Nodo ${tr.targetNodeId}`,
    );
          ...
          <div className='flex items-center gap-3 text-emerald-500'>
            ...
            <div className='flex flex-col'>
              ...
              <h2 className='font-space text-4xl font-bold text-content-primary uppercase tracking-tighter leading-none'>
                Inventario <span className='text-gradient'>Distribuido</span>
              </h2>
            </div>
          </div>
        </div>
            ...
            <div className='flex gap-4 overflow-x-auto pb-2 custom-scrollbar'>
              {nodes.map((n) => (
                <button
                  ...
                  className={`px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border shrink-0 min-h-[48px] ${selectedNode === n.id ? 'bg-blue-600 text-white border-blue-600 shadow-lg' : 'glass text-content-muted hover:text-content-primary'}`}
                >
                  {n.name}
                </button>
              ))}
                ...
                },
                {
                  ...
                  render: (item) => (
                    <div className='flex items-center justify-end gap-2'>
                      ...
                      />
                      <span
                        className={`text-[9px] font-black uppercase tracking-widest ${item.available > 0 ? 'text-emerald-500' : 'text-red-500'}`}
                      >
          ...
          <div className='space-y-10'>
            <div className='flex justify-between items-center'>
              ...
              <button
                ...
                className='px-8 py-4 bg-blue-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-blue-500 transition-all shadow-lg flex items-center gap-3'
              >
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
              {transfers.map((tr) => (
                <div
                  key={tr.id}
                  className='glass-card rounded-[3rem] p-10 border-content-muted/10 space-y-8 group hover:border-blue-500/30 transition-all bg-surface flex flex-col'
                >
                  <div className='flex justify-between items-start'>
                    <div className='space-y-1'>
                      ...
                      <div className='flex items-center gap-3'>
                        <span className='text-xl font-space font-bold text-content-primary uppercase'>
                          {tr.originNodeId}
                        </span>
                  ...
                  <div className='pt-6 border-t border-content-muted/10 flex gap-4 mt-auto'>
                    {tr.status === 'shipped' ? (
                      <button
                        onClick={() => handleConfirmReception(tr.id)}
                        className='w-full py-4 bg-emerald-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-emerald-500 transition-all shadow-xl flex items-center justify-center gap-3'
                      >
                        <FileCheck className='w-4 h-4' /> CONFIRMAR RECEPCIÓN
                      </button>
                    ) : (
                      <button className='w-full py-4 glass text-content-primary rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-white/10 transition-all'>
                        VER MANIFIESTO
                      </button>
                    )}
                  </div>
            ...
            <div className='glass-card rounded-[3rem] p-12 border-blue-500/20 bg-blue-600/[0.03] flex flex-col md:flex-row items-center justify-between gap-12 bg-surface'>
              <div className='space-y-6'>
                ...
                <p className='text-sm text-content-secondary leading-relaxed max-w-xl font-medium'>
                  El motor de IA analiza la velocidad de venta por nodo y sugiere transferencias de
                  stock para evitar quiebres de inventario locales sin necesidad de nuevas compras.
                </p>
                <EnterpriseButton onClick={runRebalanceAudit} className='px-12 py-5 bg-blue-600'>
            ...
            {rebalanceSuggestions.length > 0 && (
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
                {rebalanceSuggestions.map((s, i) => (
                  <div
                    key={i}
                    className='glass-card rounded-[2.5rem] p-8 border-content-muted/10 space-y-6 group hover:border-emerald-500/40 transition-all bg-surface shadow-sm'
                  >
                    <div className='flex justify-between items-start'>
                    ...
                    <button className='w-full py-4 glass border-content-muted/10 rounded-xl text-[9px] font-black text-content-primary uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all'>
                      EJECUTAR TRASPASO
                    </button>
                  </div>
                ))}
        ...
        <TransferModal
          ...
          onSubmit={(data) => {
            ...
            addAuditLog(
              ...
              'transfer',
              'new',
              `Solicitud de traspaso generada de ${data.originNodeId} a ${data.targetNodeId}`,
            );
      ...
      <div className='relative w-full max-w-3xl glass-card rounded-[3.5rem] border-content-muted/10 p-12 space-y-10 animate-in slide-in-from-bottom-12 duration-700 bg-surface shadow-2xl'>
        ...
        <div className='space-y-4'>
          <p className='text-[10px] font-black text-blue-500 uppercase tracking-widest'>
            Añadir Items (Búsqueda por SKU)
          </p>
          <div className='flex gap-4'>
            <input
              type='text'
              placeholder='RS-VEL-X1-38...'
              className='flex-1 p-5 glass border-content-muted/10 rounded-2xl text-content-primary bg-main/50 text-xs font-mono outline-none'
            />
            <button
              onClick={() =>
                setForm({
                  ...
                  items: [
                    ...(form.items || []),
                    { sku: 'RS-VEL-X1-38-RED', quantity: 1, name: 'Velocity X-1 Pro' },
                  ],
                })
              }
              className='px-8 py-5 bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-500 transition-all shadow-lg'
            >
              AGREGAR
            </button>
          </div>
          <div className='max-h-40 overflow-y-auto custom-scrollbar space-y-2'>
            {form.items?.map((item, i) => (
              <div
                key={i}
                className='p-4 glass rounded-xl border border-content-muted/10 flex justify-between items-center bg-main/40'
              >
                ...
                <div className='flex items-center gap-4'>
                  <input
                    type='number'
                    value={item.quantity}
                    className='w-12 bg-transparent text-center font-bold text-blue-500'
                    readOnly
                  />
                  <button
                    onClick={() =>
