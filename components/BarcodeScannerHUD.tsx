      ...
      <div className="relative w-full max-w-2xl aspect-square glass-card rounded-[3.5rem] border-white/20 overflow-hidden flex flex-col shadow-[0_0_100px_rgba(37,99,235,0.4)]">
        ...
        <div className="flex-1 flex flex-col items-center justify-center space-y-8 relative z-20">
           ...
           <div className="text-center space-y-4">
              <h3 className="text-[11px] font-black text-blue-500 uppercase tracking-[0.5em] animate-pulse">Detectando Código...</h3>
              <p className="text-[8px] text-slate-500 font-mono tracking-widest max-w-xs uppercase">Asegure el SKU dentro del visor térmico para validación instantánea.</p>
           </div>
        </div>

        <div className="p-12 border-t border-white/10 bg-white/[0.02] flex justify-between items-center relative z-20">
           <button onClick={onClose} className="px-8 py-4 glass text-slate-400 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:text-white">ABORTAR</button>
           <button onClick={simulateScan} className="px-12 py-4 bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.4em] shadow-xl hover:bg-blue-500 flex items-center gap-3">
        ...
        {hasError && (
          <div className="absolute inset-0 bg-black/90 flex flex-col items-center justify-center text-center p-12 space-y-6 z-[100]">
             ...
             <div className="space-y-2">
                <h4 className="text-2xl font-space font-bold text-white uppercase">Acceso a Hardware Denegado</h4>
                <p className="text-[10px] text-slate-500 uppercase tracking-widest leading-relaxed">No se pudo inicializar el escáner bióptico. Verifique permisos de cámara en el navegador.</p>
             </div>
             <button onClick={onClose} className="px-12 py-4 bg-red-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest">CERRAR VISOR</button>
          </div>
        )}
      </div>
