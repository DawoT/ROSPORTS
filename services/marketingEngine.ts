...
export const MarketingEngine = {
  ...
  calculateDiscountForCampaign: (items: CartItem[], camp: Campaign): number => {
    ...
    switch (camp.type) {
      ...
      case 'bogo':
        ...
        if (camp.discountValue === 2 && relevantItems.length >= 2) { // 2x1
           const sorted = relevantItems.sort((a, b) => a - b);
           return sorted[0];
        }
        if (camp.discountValue === 3 && relevantItems.length >= 3) { // 3x2
           const sorted = relevantItems.sort((a, b) => a - b);
           return sorted[0];
        }
        return 0;
   ...
   */
  verifyCouponCode: (code: string, campaigns: Campaign[], items: CartItem[]): { valid: boolean; discount: number; campaign?: Campaign; error?: string } => {
    const camp = campaigns.find(c => c.code?.toUpperCase() === code.toUpperCase() && c.status === 'active');
    
    if (!camp) return { valid: false, discount: 0, error: 'CÓDIGO NO VÁLIDO O EXPIRADO' };
    
    const discount = MarketingEngine.calculateDiscountForCampaign(items, camp);
    if (discount === 0) return { valid: false, discount: 0, error: 'NO SE CUMPLEN CONDICIONES DE MÍNIMO' };
