...
export const AuthService = {
  ...
  getOperationalLimits: (role: UserRole) => {
    switch (role) {
      case 'sales':
        return { maxDiscount: 10, voidAuthRequired: true };
      case 'supervisor':
        return { maxDiscount: 25, voidAuthRequired: false };
      case 'admin':
        return { maxDiscount: 100, voidAuthRequired: false };
      default:
        return { maxDiscount: 0, voidAuthRequired: true };
    }
  },
  ...
  hasCapability: (userCapabilities: Capability[], required: Capability): boolean => {
    return (
      ...
      userCapabilities.includes('system:manage' as Capability)
    );
  },
};
