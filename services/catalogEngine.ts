...
export const CatalogEngine = {
    ...
    return products.filter((p) => {
      ...
      const getStock = (prod: Product) =>
        prod.variants?.reduce(
          (acc, v) =>
            acc + v.inventoryLevels.reduce((lAcc, l) => lAcc + (l.quantity - l.reserved), 0),
          0,
        ) || 0;
      ...
      const matchesSize =
        filters.sizes.length === 0 || p.variants?.some((v) => filters.sizes.includes(v.size));
      const matchesColor =
  ...
  sort: (products: Product[], option: SortOption): Product[] => {
    ...
    switch (option) {
      ...
      case 'best_sellers':
        return sorted.sort((a, b) => (b.tractionScore || 0) - (a.tractionScore || 0));
      default:
        return sorted;
    }
  },
